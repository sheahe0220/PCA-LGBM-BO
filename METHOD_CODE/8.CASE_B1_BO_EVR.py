...
# 8.CASE_B1_BO_EVR.py
# ------------------------------------------------------------
# Case B-1: GP-BO with random init + EVR-weighted PC distance (J) objective
# Purpose: isolate the objective-function effect (vs Case B-2 RMSE)
...
RESULT_ROOT  = BASE / "RESULTS" / "CaseB1_EVR"
...
# ===== BO settings (IDENTICAL to Case B-2) =====
...
# PCA preprocessing (match training pipeline exactly)
N_POINTS = 480
...
class CostTracker:
    ...
    def to_dict(self):
        return {
            ...
            "mode": "CaseB1_random_EVR_weighted",
        }
...
def preprocess_depth_series(df: pd.DataFrame) -> np.ndarray:
    """SWMM depth -> NaN interp -> clip neg -> resample 480. No baseline removal."""
    ...
    # No baseline removal (SWMM starts at 0, matches PCA training)
    x = np.maximum(x, 0.0)
...
def main():
    ...
    log("=" * 80)
    log("Case B-1: Random Init + EVR-weighted PC distance (J)")
    ...
    # [2/4] Random init (same seed as B-2)
    ...
    t0 = time.perf_counter()
    J_total, J_events = evaluate_theta(theta_dict, "init000", pca_models, obs_pcs, evr_weights)
    ...
    try:
        ...
        ax.set_title("Case B-1: Random Init + EVR-weighted Objective")
        ax.legend()
