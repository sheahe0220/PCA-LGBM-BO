# -*- coding: utf-8 -*-
# 11.CASE_C2_HYBRID_RMSE.py
# ------------------------------------------------------------
# Case C-2 (Hybrid): LGBM warm-start + direct time-domain RMSE objective
# Init point = Case A LGBM pooled_theta_optimal
# Objective = direct time-domain RMSE (same as B-2)
...
RESULT_ROOT  = BASE / "RESULTS" / "CaseC2_HYBRID_RMSE"
...
# ===== BO settings (IDENTICAL to B-1/B-2/C-1) =====
...
class CostTracker:
    ...
    def to_dict(self):
        return {
            ...
            "mode": "CaseC2_warmstart_direct_RMSE",
        }
...
def main():
    ...
    log("=" * 80)
    log("Case C-2: LGBM Warm-Start + Direct RMSE objective")
    ...
    # [2/4] LGBM warm-start init (KEY DIFFERENCE from Case B-2)
    ...
    t0 = time.perf_counter()
    J_total, J_events = evaluate_theta(theta_dict, "init_lgbm", obs_minbase)
    ...
    try:
        ...
        ax.set_ylabel("J (Direct RMSE, m)")
        ax.set_title("Case C-2: LGBM Warm-Start + Direct RMSE Objective (min-baseline)")
        ax.legend()
