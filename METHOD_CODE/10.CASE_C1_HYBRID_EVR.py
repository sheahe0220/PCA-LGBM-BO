# -*- coding: utf-8 -*-
# 10.CASE_C1_HYBRID_EVR.py
# ------------------------------------------------------------
# Case C-1 (Hybrid): LGBM warm-start + EVR-weighted PC distance (J)
# Init point = Case A LGBM pooled_theta_optimal
# Objective = EVR-weighted RMSE in PC space (same as B-1)
# Uses min-baseline OBS PCA
# ------------------------------------------------------------
from __future__ import annotations
import os, time, json, re, shutil, signal, pickle
...
OBS_DIR      = BASE / "input" / "obs_depth"

# PCA models / observed PC scores / EVR weights (from 5.PCA.py, 6.PCA_OBS.py)
PCA_ROOT      = BASE / "PCA"
PCA_MODEL_DIR = PCA_ROOT / "models"
OBS_PC_DIR    = PCA_ROOT / "obs_pc"
EVR_PATH      = PCA_ROOT / "PC_selected_evr.csv"

# LGBM warm-start theta (from 7.CASE_A_LGBM.py)
LGBM_THETA_PATH = BASE / "RESULTS" / "CaseA_LGBM" / "results" / "pooled_theta_optimal.json"

RESULT_ROOT  = BASE / "RESULTS" / "CaseC1_HYBRID_EVR"
...
# ===== BO settings (IDENTICAL to B-1/B-2) =====
...
N_POINTS = 480  # PCA preprocessing length
...
class CostTracker:
    ...
    def to_dict(self):
        return {
            ...
            "mode": "CaseC1_warmstart_EVR_weighted",
        }
...
def load_evr_weights() -> dict:
    ...
    return {row["target"]: float(row["evr"]) for _, row in df.iterrows()}


# ===== Load LGBM warm-start theta =====
def load_lgbm_warmstart() -> np.ndarray:
    with open(LGBM_THETA_PATH, "r") as f:
        raw = json.load(f)
    key_map = {"Imperv_scale": "Imperv", "Width_scale": "Width",
               "Nimp_scale": "Nimp", "n_pipe_scale": "n_pipe"}
    x0 = []
    for feat in FEATURES:
        raw_key = key_map[feat]
        val = float(raw[raw_key])
        lo, hi = BOUNDS[FEATURES.index(feat)]
        val = np.clip(val, lo, hi)
        x0.append(val)
    return np.array(x0)


# ===== Preprocessing (match PCA training: no baseline removal) =====
def preprocess_depth_series(df: pd.DataFrame) -> np.ndarray:
    ...
    return np.interp(new_pos, orig_pos, x)


# ===== Objective: EVR-weighted PC distance =====
...
def evaluate_theta(theta_dict: dict, iter_id: str,
                   pca_models: dict, obs_pcs: dict, evr_weights: dict):
...
def main():
    ...
    log("=" * 80)
    log("Case C-1: LGBM Warm-Start + EVR-weighted PC distance (J)")
    ...
    # [1/4] Load data
    print(f"\n[1/{N_TOTAL_STEPS} | 25%] Loading data...")
    pca_models  = load_pca_models()
    obs_pcs     = load_obs_pcs()
    evr_weights = load_evr_weights()
    log(f"  PCA models: {list(pca_models.keys())}")
    log(f"  EVR weights: {evr_weights}")

    # [2/4] LGBM warm-start init (KEY DIFFERENCE from Case B-1)
    print(f"\n[2/{N_TOTAL_STEPS} | 50%] LGBM warm-start init...")
    COST.mark_init_start()

    x0 = load_lgbm_warmstart()
    theta_dict = {f: float(v) for f, v in zip(FEATURES, x0)}
    log(f"  LGBM theta (warm-start): {theta_dict}")

    t0 = time.perf_counter()
    J_total, J_events = evaluate_theta(theta_dict, "init_lgbm", pca_models, obs_pcs, evr_weights)
    ...
    log(f"  [INIT_LGBM] J_best={J_best:.6f}")
    ...
    try:
        for it in tqdm(range(1, N_ITER + 1), desc="[BO] Optimization"):
            ...
            try:
                J_next, J_events = evaluate_theta(theta_dict, f"bo{it:03d}",
                                                   pca_models, obs_pcs, evr_weights)
    ...
    best_payload = {
        ...
        "mode": "CaseC1_warmstart_EVR_weighted",
        "init_source": "LGBM_pooled_theta_optimal",
    }
    ...
    try:
        ...
        ax.set_ylabel("J (EVR-weighted PC distance)")
        ax.set_title("Case C-1: LGBM Warm-Start + EVR-weighted Objective")
        ax.legend()
